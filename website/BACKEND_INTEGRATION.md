...
- `"same_or_neutral"`：接受同侧或中立（4）的候选人。
- `"different"`：只接受与本人位于相反侧的候选人。
- `"different_or_neutral"`：接受异侧或中立（4）的候选人。

严格值（`same` / `different`）会拒绝答 4 的候选人；带 `_or_neutral` 的宽松值接受。
硬性条件会叠乘：22男/22女的池子里，每人各设 1 条严格偏好就有 6 人落单，设 8 条时
42 人落单；同样条件改用宽松值则分别是 0 人和 6 人。
...
| --- | --- | --- | --- |
| `any` | 任意 | 任意 | 是 |
| 任意侧别偏好 | 4 | 任意 | 是，A 为中立时该题不设限制 |
| `same` | 左或右 | 与 A 同侧 | 是 |
| `same` | 左或右 | 与 A 异侧或为 4 | 否 |
| `same_or_neutral` | 左或右 | 与 A 同侧，或为 4 | 是 |
| `same_or_neutral` | 左或右 | 与 A 异侧 | 否 |
| `different` | 左或右 | 与 A 异侧 | 是 |
| `different` | 左或右 | 与 A 同侧或为 4 | 否 |
| `different_or_neutral` | 左或右 | 与 A 异侧，或为 4 | 是 |
| `different_or_neutral` | 左或右 | 与 A 同侧 | 否 |
...
- 校验偏好值只能为 `"any"`、`"same"`、`"same_or_neutral"`、`"different"` 或 `"different_or_neutral"`；服务端最终数据应将本人答案为 4 的题归一化为 `"any"`。
- 在候选人进入现有排序前执行上述双向硬性筛选。

以上后端实现已完成（持久化、校验、归一化、双向硬性筛选），逐题偏好现在会实际影响匹配结果。
